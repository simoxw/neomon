              ...
              return (
                <div key={mId} className="relative">
                  <button
                    ...
                    onClick={() => handleAction('move', mId)}
                    onMouseEnter={() => setHoveredMove(moveData)}
                    onMouseLeave={() => setHoveredMove(null)}
                    onTouchStart={() => setHoveredMove(moveData)}
                    onTouchEnd={() => setTimeout(() => setHoveredMove(null), 1500)}
                    className="w-full h-full bg-white/5 border border-white/10 rounded-2xl flex flex-col items-center justify-center p-3 hover:bg-emerald-600 hover:text-white transition-all disabled:opacity-30 group active:scale-95 shadow-lg"
                  >
                  ...
                  <MoveTooltip move={moveData} visible={hoveredMove?.id === moveData.id} />
                </div>
              );
            })}
          </div>
